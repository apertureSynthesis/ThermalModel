    ...
    def getCombinedModel(self):
        ...
        if self.pars['doCombinedModel']:
            ...
            for gamma, emis, modelDir in records:
                modelFiles = sorted(glob(os.path.join(modelDir, '*_rt_???.fits')))
                if not modelFiles:
    ...
    def plotCombinedModel(self):
        ...
        if self.pars['doModelPlotting']:
            ...
            for i in range(len(gamma)):
                ax.plot(lonVals, flux[i, 2, :, 2, -2], 'o',label=f"TI={gamma[i]:.1f}")
            ax.text(0.8, 0.1, f"$\\epsilon$ = {emis[2]:.2f}", transform = ax.TransAxes)
            ax.text(0.8, 0.06, f"n = {refIdx[i]:.2f}", transform = ax.TransAxes)
            ax.text(0.8, 0.02, f"$\\tan\\Delta$ = {loss[-2]:.2f}", transform = ax.TransAxes)
